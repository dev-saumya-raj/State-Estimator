...
class KalmanNet(keras.Model):
  def __init__(self, f, h, n, m, dt=1e-2):
    ...
    self.dense_x_hat = keras.layers.Dense(100, activation='relu')
    self.dense_x_tilde = keras.layers.Dense(100, activation='relu')
    self.dense_Sxx_in = keras.layers.Dense(100, activation='relu')
    self.dense_Sxx = keras.layers.Dense(n*n)
    self.dense_Sxx_out = keras.layers.Dense(50, activation='relu')
    self.dense_Y = keras.layers.Dense(30, activation='relu')
    self.dense_K = keras.layers.Dense(n*m, activation='relu')
  ...
  def call(self, inputs, training=False):
    ...
    for t in range(tsteps):
      ...
      p = del_x_hat
      p = self.dense_x_hat(p)
      Q, gQ = self.gru_Q(p, gQ)

      p = self.dense_x_tilde(del_x_tilde)
      ...
      Syy, gSyy = self.gru_Syy(p, gSyy)

      p = tf.concat((Sxx, Syy), axis=1)
      K = self.dense_K(p)

      p = tf.concat((Syy, K), axis=1)
      p = self.dense_1(p)
